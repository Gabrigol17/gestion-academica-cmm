...
class AcudienteController:
    ...
    def crear(self, data):
        ...
        acudiente = self.acudiente_service.crear(data['ACU_NOMBRES'], data['ACU_APELLIDOS'], data['ACU_ESTADO'])
        if acudiente is None:
            return jsonify({'mensaje': 'No se pudo crear el acudiente'}), 500
        return jsonify({'mensaje': 'Acudiente creado exitosamente', 'acudiente': acudiente}), 201

    def actualizar(self, acu_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.acudiente_service.actualizar(data['ACU_NOMBRES'], data['ACU_APELLIDOS'], data['ACU_ESTADO'], acu_uuid):
            return jsonify({'mensaje': 'Acudiente no encontrado'}), 404
        return jsonify({'mensaje': 'Acudiente actualizado exitosamente'}), 200

    def eliminar(self, acu_uuid):
        if not self.acudiente_service.eliminar(acu_uuid):
            return jsonify({'mensaje': 'Acudiente no encontrado'}), 404
