...
class AcudienteEstudianteController:
    ...
    def crear(self, data):
        ...
        relacion = self.acudiente_estudiante_service.crear(
        ...
        if relacion is None:
            return jsonify({'mensaje': 'No se pudo crear la relación acudiente-estudiante'}), 500
        return jsonify({'mensaje': 'Relación acudiente-estudiante creada exitosamente', 'acudiente_estudiante': relacion}), 201

    def actualizar(self, acu_est_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.acudiente_estudiante_service.actualizar(
            data['ACU_EST_PARENTESCO'],
            data['ACU_EST_ACU_ID'],
            data['ACU_EST_EST_ID'],
            data['ACU_EST_ESPRINCIPAL'],
    ...
    def eliminar(self, acu_est_uuid):
        if not self.acudiente_estudiante_service.eliminar(acu_est_uuid):
            return jsonify({'mensaje': 'Relación acudiente-estudiante no encontrada'}), 404
