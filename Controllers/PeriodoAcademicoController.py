...
class PeriodoAcademicoController:
    ...
    def crear(self, data):
        ...
        periodo = self.periodo_academico_service.crear(
            data['PER_ACA_NUMERO'],
        ...
        if periodo is None:
            return jsonify({'mensaje': 'No se pudo crear el período académico'}), 500
        return jsonify({'mensaje': 'Período académico creado exitosamente', 'periodo_academico': periodo}), 201

    def actualizar(self, per_aca_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.periodo_academico_service.actualizar(
            data['PER_ACA_NUMERO'],
            data['PER_ACA_FECHA_INICIO'],
            data['PER_ACA_FECHA_FIN'],
            data['PER_ACA_VIG_ID'],
    ...
    def eliminar(self, per_aca_uuid):
        if not self.periodo_academico_service.eliminar(per_aca_uuid):
            return jsonify({'mensaje': 'Período académico no encontrado'}), 404
