...
class MatriculaController:
    ...
    def crear(self, data):
        ...
        matricula = self.matricula_service.crear(data['MATR_EST_ID'], data['MATR_CUR_VIG_ID'])
        if matricula is None:
            return jsonify({'mensaje': 'No se pudo crear la matrícula'}), 500
        return jsonify({'mensaje': 'Matrícula creada exitosamente', 'matricula': matricula}), 201

    def actualizar(self, matr_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.matricula_service.actualizar(data['MATR_EST_ID'], data['MATR_CUR_VIG_ID'], matr_uuid):
            return jsonify({'mensaje': 'Matrícula no encontrada'}), 404
        return jsonify({'mensaje': 'Matrícula actualizada exitosamente'}), 200

    def eliminar(self, matr_uuid):
        if not self.matricula_service.eliminar(matr_uuid):
            return jsonify({'mensaje': 'Matrícula no encontrada'}), 404
