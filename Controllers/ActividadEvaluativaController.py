...
class ActividadEvaluativaController:
    ...
    def crear(self, data):
        ...
        actividad = self.actividad_evaluativa_service.crear(
            data['ACT_EVA_NOMBRE'],
        ...
        if actividad is None:
            return jsonify({'mensaje': 'No se pudo crear la actividad evaluativa'}), 500
        return jsonify({'mensaje': 'Actividad evaluativa creada exitosamente', 'actividad_evaluativa': actividad}), 201

    def actualizar(self, act_eva_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.actividad_evaluativa_service.actualizar(
            data['ACT_EVA_NOMBRE'],
            data['ACT_EVA_DESCRIPCION'],
            data['ACT_EVA_ASIG_ACA_ID'],
            data['ACT_EVA_COM_EVA_ID'],
    ...
    def eliminar(self, act_eva_uuid):
        if not self.actividad_evaluativa_service.eliminar(act_eva_uuid):
            return jsonify({'mensaje': 'Actividad evaluativa no encontrada'}), 404
