...
class GradoController:
    ...
    def crear(self, data):
        ...
        grado = self.grado_service.crear(data['GRAD_NOMBRE'], data['GRAD_NIV_EDUC_ID'])
        if grado is None:
            return jsonify({'mensaje': 'No se pudo crear el grado'}), 500
        return jsonify({'mensaje': 'Grado creado exitosamente', 'grado': grado}), 201

    def actualizar(self, grad_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.grado_service.actualizar(data['GRAD_NOMBRE'], data['GRAD_NIV_EDUC_ID'], grad_uuid):
            return jsonify({'mensaje': 'Grado no encontrado'}), 404
        return jsonify({'mensaje': 'Grado actualizado exitosamente'}), 200

    def eliminar(self, grad_uuid):
        if not self.grado_service.eliminar(grad_uuid):
            return jsonify({'mensaje': 'Grado no encontrado'}), 404
