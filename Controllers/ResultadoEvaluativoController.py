...
class ResultadoEvaluativoController:
    ...
    def crear(self, data):
        ...
        resultado = self.resultado_evaluativo_service.crear(
            data['RES_EVA_NOTA'],
            data['RES_EVA_AJUSTE'],
        ...
        if resultado is None:
            return jsonify({'mensaje': 'No se pudo crear el resultado evaluativo'}), 500
        return jsonify({'mensaje': 'Resultado evaluativo creado exitosamente', 'resultado_evaluativo': resultado}), 201

    def actualizar(self, res_eva_uuid, data):
        if not data or not all(campo in data for campo in self.CAMPOS_REQUERIDOS):
            return jsonify({'mensaje': 'Faltan campos requeridos'}), 400

        if not self.resultado_evaluativo_service.actualizar(
            data['RES_EVA_NOTA'],
            data['RES_EVA_AJUSTE'],
            data['RES_EVA_OBSERVACION'],
            data['RES_EVA_MAT_ID'],
    ...
    def eliminar(self, res_eva_uuid):
        if not self.resultado_evaluativo_service.eliminar(res_eva_uuid):
            return jsonify({'mensaje': 'Resultado evaluativo no encontrado'}), 404
